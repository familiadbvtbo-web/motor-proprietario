 ...
 *
 * VERSÃO REVISADA:
 * - alimenta o MTF com vários timeframes reais, em vez de somente o selecionado;
 * - respeita o intervalo de atualização de cada timeframe;
 * - não inventa bid/ask quando a fonte fornece somente last price;
 * - impede que cobertura MTF incompleta seja tratada como confluência completa;
 * - reduz dupla penalização de FSI/neutralidade;
 * - aplica gate de qualidade antes de liberar COMPRA/VENDA;
 * - mantém histórico/calibração somente como evidência auxiliar;
 * - evita bloquear a UI durante chamadas da API.
 */
class MainActivity : AppCompatActivity() {
    ...
    /*
     * Y1 é suportado pelo TwelveDataCandleClient, mas não é usado
     * como timeframe operacional. Ele pode entrar no contexto de longo prazo.
     */
    private val mtfTimeframes = listOf(
        "M1", "M5", "M15", "M30", "H1", "H4", "D1", "W1", "MN1", "Y1"
    )

    /*
     * No máximo 3 consultas por ciclo. Isso reduz o risco de 429 e mantém
     * o aplicativo responsivo. Timeframes ausentes têm prioridade.
     */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dopmDashboardController.setSelectionListeners(
            ...
            assetChanged = { asset ->
                if (asset.isNotBlank() && asset != selectedAsset) selectAsset(asset)
            },
    ...
    private fun connectRealtime() {
        quoteClient.connect(
            ...
            onError = {
                runOnUiThread { dopmDashboardController.view()?.setOnline(false) }
            }
        )
    }
    ...
    private fun analyzeMarket() {
        ...
        thread {
            try {
                ...
                val candidateDirection = finalDirection(
                ...
                val sequence = SequenceEngine.advance(
                    sequenceStage,
                    SequenceInput(
                        signalDetected = candidateDirection != "NEUTRO",
                        confirmation = effectiveMtf >= 60.0 && mtfResult.coverage >= 60.0,
                        continuation = when (candidateDirection) {
                        ...
                        invalidated = effectiveFsi >= 80.0 || realtime.market.dataQuality != "GOOD"
                    )
                )
                ...
                /*
                 * GATE FINAL:
                 * a direção candidata só vira decisão depois de passar pelos
                 * gates de qualidade, FSI, sequência, MTF, determinismo,
                 * dominância e conflito direcional.
                 */
                val bestDirection = decisionGate(
                    ...
                    dataQuality = realtime.market.dataQuality,
                    deterministic = bestDeterministic,
                    sequenceConfirmed = sequence.confirmed
                )
    ...
    private fun effectiveFsi(
        ...
        /* Histórico é evidência auxiliar, nunca substitui o risco atual. */
        return (
            realtimeFsi * 0.75 +
    ...
    /**
     * Atualiza uma pequena quantidade de timeframes por ciclo.
     * O cache persistente é usado primeiro; uma consulta só ocorre quando
     * o candle daquele timeframe realmente venceu o seu intervalo.
     */
    private fun updateCandles(now: Long) {
        ...
        /* Primeiro o timeframe selecionado e os principais contextos. */
        ordered.add(selectedTimeframe.uppercase())
        ordered.addAll(listOf("M15", "M5", "M30", "H1", "H4", "D1", "W1", "MN1", "M1", "Y1"))
        ordered.addAll(mtfTimeframes)

        for (tf in ordered.distinct()) {
            if (tf in mtfTimeframes && tf !in candidates) candidates.add(tf)
        }
        ...
        val rotated = due.drop(candleRoundRobin % due.size) + due.take(candleRoundRobin % due.size)
        var requested = 0
        ...
        for (timeframe in rotated) {
            ...
            try {
                val persistent = marketCandleCache.get(selectedAsset, timeframe)
                if (persistent.isNotEmpty()) {
                    synchronized(candleCache) { candleCache[timeframe] = persistent }
                }
                ...
                if (!cached.isNullOrEmpty() && now - last < interval) continue
                ...
                if (fresh.isNotEmpty()) {
                    ...
                    synchronized(candleCache) { candleCache[timeframe] = merged }
                    lastCandleUpdate[timeframe] = now
                }
            } catch (error: Exception) {
                val message = error.message?.trim()?.take(180) ?: "ERRO_DESCONHECIDO"
                if (message.contains("429", ignoreCase = true)) {
                    candleApiBackoffUntil = System.currentTimeMillis() + candleApiBackoffMs
                    break
                }
                /* Um timeframe defeituoso não derruba a análise dos demais. */
            }
        }
    }
    ...
    private fun combineFinalProbabilities(
        ...
        /*
         * FSI reduz direção de forma progressiva. Não somamos o FSI inteiro
         * ao neutro, evitando o problema antigo de "NEUTRO" dominar a saída.
         */
        val directionalFactor = (1.0 - risk * 0.45).coerceIn(0.55, 1.0) * mtfFactor
        buy *= directionalFactor
        sell *= directionalFactor

        neutral = neutral * (0.80 + risk * 0.20) + risk * 8.0 + (1.0 - mtf / 100.0) * 4.0

        val total = max(0.0, buy) + max(0.0, sell) + max(0.0, neutral)
        if (total <= 0.0 || !total.isFinite()) return Triple(33.33, 33.33, 33.34)
    ...
    private fun finalDirection(
        ...
        if (dataQuality != "GOOD" ||
            mtfCoverage < 60.0 ||
            falseSignalRisk >= 55.0 ||
            deterministic.confirmation < 50.0
        ) return "NEUTRO"
    ...
    private fun decisionGate(
        ...
        val buy = probabilities.first.coerceIn(0.0, 100.0)
        val sell = probabilities.second.coerceIn(0.0, 100.0)
        val neutral = probabilities.third.coerceIn(0.0, 100.0)
        val risk = falseSignalRisk.coerceIn(0.0, 100.0)
        val mtf = mtfConfluence.coerceIn(0.0, 100.0)
        val coverage = mtfCoverage.coerceIn(0.0, 100.0)
        val det = deterministic.confidence.coerceIn(0.0, 100.0)

        if (dataQuality != "GOOD") return "NEUTRO"
        if (risk >= 65.0) return "NEUTRO"
        if (!sequenceConfirmed) return "NEUTRO"
        if (mtf < 50.0 || coverage < 60.0) return "NEUTRO"
        if (det < 40.0) return "NEUTRO"

        val values = listOf(buy, sell, neutral).sortedDescending()
        val dominance = values[0] - values[1]
        val buyConflict = deterministic.sellScore > deterministic.buyScore + 20.0
        val sellConflict = deterministic.buyScore > deterministic.sellScore + 20.0

        if (candidateDirection == "COMPRA" &&
            buy >= 70.0 &&
            buy > sell + 8.0 &&
            buy > neutral &&
            dominance >= 8.0 &&
    ...
    private fun findBestTimeframe(
        ...
        /* Timeframes operacionais; os maiores continuam como contexto MTF. */
        val candidates = listOf("M5", "M15", "M30", "H1")
        var best = if (realtime.metrics.containsKey(selectedTimeframe)) selectedTimeframe else "M15"
        ...
        for (timeframe in candidates) {
            ...
            val final = combineFinalProbabilities(prob, det, realtime.fsi, mtfConfluence)
            val directional = max(final.first, final.second)
            val score = directional + det.confidence * 0.20 + det.confirmation * 0.15 - final.third * 0.25
    ...
    private fun timeframeRank(timeframe: String): Int = when (timeframe.uppercase()) {
        "M1" -> 1
        "M5" -> 2
        "M15" -> 3
        "M30" -> 4
    ...
    private fun fibonacciEvidence(candles: List<MarketCandle>?, bullish: Boolean): Double {
    ...
    private fun updateMainScreen(
        ...
        dopmDashboardController.updateMathematics(
            max(buy, sell), deterministic.confidence, effectiveMtfConfluence
        )
        dopmDashboardController.updateIndicators(
            realtime.metrics[bestTimeframe] ?: realtime.metrics[selectedTimeframe] ?: realtime.metrics["M15"] ?: return,
            realtime.fsi,
        ...
        if (direction == "NEUTRO" || !entryPlan.valid) {
            ...
            dopmDashboardController.updateTiming("AGUARDAR", "--")
        } else {
